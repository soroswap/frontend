...
export const getPairsInfo = async (currencies: [TokenType | undefined, TokenType | undefined][], sorobanContext: SorobanContextType) => {
    ...
    return await Promise.all(
      pairReserves.map(async (reserves, i) => {
        ...
        const liquidityToken = {
          ...
          userBalance: await tokenBalance(pairAddresses[i], sorobanContext.address ?? "", sorobanContext),
          totalSupply
        }
          
        const userShare0 = (BigNumber(reserves.reserve0 as BigNumber).multipliedBy(BigNumber(liquidityToken.userBalance))).dividedBy(BigNumber(totalSupply))
        const userShare1 = (BigNumber(reserves.reserve1 as BigNumber).multipliedBy(BigNumber(liquidityToken.userBalance))).dividedBy(BigNumber(totalSupply))
...
export const getPairInfo = async (tokenA: TokenType | undefined, tokenB: TokenType | undefined, sorobanContext: SorobanContextType) => {
