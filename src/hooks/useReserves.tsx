...
export async function reservesBigNumber(
  ...
  const reserves = scValStrToJs(reserves_scval?.xdr as string)

  return {
    reserve0: BigNumber(reserves as string[0]),
    reserve1: BigNumber(reserves as string[1]),
  };
}
...
export async function reservesBNWithTokens(
  ...
  const token0 = scValStrToJs(token0_scval?.xdr as string)
  ...
  const token1 = scValStrToJs(token1_scval?.xdr as string)
